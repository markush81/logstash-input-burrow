...
This Logstash input plugin allows you to call Burrow API, decode the output of it into event(s), and
...
Using the burrow input plugin with a custom CA or self signed cert.
...
==== burrow Input Configuration Options
...
| <<plugins-{type}s-{plugin}-client_config>> |<<hash,hash>>|Yes
...
[id="plugins-{type}s-{plugin}-client_config"]
===== `client_config`
...
A Hash of client_config in this format : url => "http://localhost:8000". url is mandatory.
